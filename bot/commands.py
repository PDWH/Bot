...
configMysql = {
    'user': '823923', #uzytkownik bazy danych
    'password': 'gThX7OS0V3l21R', #haslo do bazy danych
    'host': 'sql.pukawka.pl', #ip bazy danych
    'database': '823923_', #baza danych
}
rankSynchro = 840627411900432434 # id nadawanej rangi na discordzie (do wyciągnięcia przez tryb developerski)
    ...
    async def synchronizacja(self, ctx, code = None):
        ...
        if code is not None:
            ...
            for row in records:
                znaleziono = True
                if str(row[2]) == "nie":
                    cursor.execute("select `login` from pystories_users WHERE id='"+str(row[0])+"'")
